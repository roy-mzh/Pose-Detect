...
class Test:
    def __init__(self):
        ...
        self.model2.load_state_dict(torch.load("checkpoint_.params")["model_state_dict"], strict = False)
        self.model2.eval()
...
def get_result(x, thred):
    t = np.array(x)
    return int((t <= thred)[0][0])
    ...
    def detect(self):
        while self.flag:
            ...
            if tmp is None:
                ...
                continue
            mask   = self.net(tmp)
            print(nn.Sigmoid()(torch.tensor(mask).float()))
            mask   = torch.argmax(nn.functional.softmax(torch.tensor(mask).float()))
            # mask   = get_result(nn.functional.sigmoid(torch.tensor(mask).float()), 0.5)
            print(mask)
            # cv2.imshow("", tmp)
